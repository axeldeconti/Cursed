...
public class AiController : MonoBehaviour
{
    ...
    private bool PlayerInRange(float range, bool raycastOn)
    {
        ...
        {
            if (raycastOn && !Physics2D.Linecast(transform.position + Vector3.up, player.transform.position + Vector3.up, _ray.collisionMask))
            {
                return true;
            }
            else if (!raycastOn)
        ...
        Debug.DrawLine(transform.position + Vector3.up, player.transform.position + Vector3.up);
